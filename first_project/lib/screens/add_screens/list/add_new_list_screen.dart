...
class _AddNewListScreenState extends State<AddNewListScreen> {
  ...
  Future<void> _saveNewList(BuildContext context, AppModel appState) async {
    ...
    if (input.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please enter a name for your list")));
      return;
    }
    ...
    final newListId = await appState.createList(input, widget.collectionId, finalImagePath, _selectedColorId);
    ...
    if (mounted) {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => ListScreen(listId: newListId, listName: input)),
      );
    }
  }
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: selectedColor, width: 2)),
                ),
              ),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: _imageFile != null
            ...
            : const Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.add_a_photo, color: Colors.white70, size: 48), Text("Add List Cover Photo", style: TextStyle(color: Colors.white70))]),
      ),
    );
  }

  Widget _buildColorPickerSection() {
    return Consumer<AppModel>(builder: (context, appState, _) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Card Theme Color", style: TextStyle(color: Colors.white70, fontSize: 16)),
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    width: 55,
                    decoration: BoxDecoration(color: color, shape: BoxShape.circle, border: Border.all(color: _selectedColorId == entry.key ? Colors.white : Colors.transparent, width: 3)),
                    child: _selectedColorId == entry.key ? const Icon(Icons.check, color: Colors.white) : null,
                  ),
                );
              },
            ),
  ...
  Widget _buildTagSection(Color themeColor) {
    return Consumer<AppModel>(builder: (context, appState, _) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Tags", style: TextStyle(color: Colors.white70, fontSize: 16)),
          const SizedBox(height: 12),
          Wrap(
            spacing: 8,
            children: [
              ...appState.tags.map((tag) => FilterChip(
                label: Text(tag.name),
                selected: _selectedTagIds.contains(tag.id),
                onSelected: (val) => setState(() => val ? _selectedTagIds.add(tag.id) : _selectedTagIds.remove(tag.id)),
  ...
  void _showCreateTagDialog(AppModel appState) async {
    final name = await showDialog<String>(context: context, builder: (_) => InputDialog(title: "Create new Tag"));
  ...
  void _showPickerOptions(BuildContext context) { /* Keep your original modal code here */ }
}